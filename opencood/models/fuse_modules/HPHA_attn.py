...
'''
TODO add communication rate and other realization about multi-batch processing
'''
import numpy as np
import random
import torch
...
class HPHA(nn.Module):
    ...
    def forward(self, x, historical_x, psm_single, record_len, pairwise_t_matrix, orin_time_delay, backbone=None):
        ...
        _, C, H, W = x.shape # [N, 64, 200, 704] grid size determined with both voxel size and lidar range
        ...
        truely_time_delay = orin_time_delay[0:sparse_maps.shape[0]]
        ...
        ups.append(historical_x[0].unsqueeze(0)) # [(1, 128, ..)*3 , (1, 64, ..)*2] Seems one 128* layer is abundent
        ups.append(historical_x[1].unsqueeze(0))
        # print(len(ups))
        if len(ups) > 1:
            x_fuse = torch.cat(ups, dim=1) # [1, 512, ...]
        elif len(ups) == 1:
            x_fuse = ups[0]
