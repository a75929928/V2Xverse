...
class PointPillarSelect2Col(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        spatial_features_2d = batch_dict['spatial_features_2d'] # [5, 384, 100, 352]
        # Down-sample feature to reduce memory
        if self.shrink_flag: ## self.shrink_flag->True
            spatial_features_2d = self.shrink_conv(spatial_features_2d) # [5, 256, 50, 176]
        psm_single = self.cls_head(spatial_features_2d) # [5, 2, 50, 176]

        # Compressor
        if self.compression:
            # The ego feature is also compressed
            spatial_features_2d = self.naive_compressor(spatial_features_2d) # [5, 256, 50, 176]
        ...
        fused_feature = self.fusion_net(batch_semantic_informantion_dict, ## semantic information [3, 64, 200, 704]
                                                             batch_dict['spatial_features'][1:3], ## historical semantic information of ego [2, 64, 200, 704]
                                                             psm_single, # [5, 2, 50, 176]
                                                             record_len,
                                                             pairwise_t_matrix,
                                                             time_delay,
                                                             self.backbone)
        if self.shrink_flag:
            # Simply Ignore first layer of fusion output
            if fused_feature.shape[1] != 384:
                # import math
                # N, C, H ,W = fused_feature.shape # [1, 512, 96, 352]
                # x = int((N * C * H * W) / (4 * 384 * 64))
                # fused_feature = fused_feature.view(4, 384, 64, x)
                fused_feature = fused_feature[:, 128:, :, :]
        ...
        output_dict.update({'fused_feature': fused_feature})
        return output_dict
