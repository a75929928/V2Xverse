...
# Rename since original point pillar loss differ
class PointPillarLossSelect2Col(nn.Module):
    def __init__(self, args):
        super(PointPillarLossSelect2Col, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        """
        rm = output_dict['rm']
        psm = output_dict['psm'] # [4, 2, 32, 72]
        targets = target_dict['targets'] # (48, 144, 14)

        cls_preds = psm.permute(0, 2, 3, 1).contiguous() # [4, 32, 72, 2]

        box_cls_labels = target_dict['pos_equal_one'] # torch.Size([1, 48, 144, 2]) determined by feature map shape
        ...
        targets = targets.view(rm.size(0), -1, 7) # should be align with rm, so 
