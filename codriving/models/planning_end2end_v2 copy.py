...
class WaypointPlanner_e2e_v2(nn.Module):
    """
    ...
    def forward(self, input_data : Dict) -> torch.Tensor:
        ...
        Args:
            input_data: input data to forward
                ...
                - feature: BEV feature map from perception module
                - target: target point to go to

        Return:
            torch.Tensor: predicted waypoints
        """
        # only difference from v1: cliped the first dimension of occupancy
        occupancy = input_data["occupancy"][:,:,1:,:,:]  # B,T,C,H,W = ([2, 5, 6, 192, 96])
        batch, seq, c, h, w = occupancy.size()

        x = occupancy.view(-1, c, h, w)  # batch*seq, c, h, w ([10, 6, 192, 96])
        x = F.relu(self.bn_pre_1(self.conv_pre_1_no_occ(x))) # ([batch*seq, 32, 192, 96]) ([10, 32, 192, 96])
