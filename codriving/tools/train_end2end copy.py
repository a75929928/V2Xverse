        ...
        try:
            ...
            with torch.no_grad():
                ...
                for frame in frame_list:
                    ...
                    # print('original \'fused_feature\': ', perception_results['fused_feature'].shape) # torch.Size([8, 128, 96, 288])
                ...
                for b in range(len(perception_results_list[0]['fused_feature'])):
                    ...
                    # print('feature_to_warp: ', feature_to_warp.shape) # torch.Size([8, 128, 96, 288])

                    for t in range(5):
                        feature_to_warp[0, t, :] = pred_batch_data['fused_feature'][t][b] # occ_map_list[t]
                        
                        ## align with for loop
                        # if len(pred_batch_data['detmap_pose'].shape) > 2:
                        #     pred_batch_data['detmap_pose'] = pred_batch_data['detmap_pose'][0].view()
                            
                        det_map_pose[:, t] = torch.tensor(pred_batch_data['detmap_pose'][b,t]) # N, 3
                    
                    feature_warped = warp_image(det_map_pose, feature_to_warp)

                    print('feature_warped: ', feature_warped.shape)
