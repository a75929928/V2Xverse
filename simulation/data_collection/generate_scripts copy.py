...
routes = {}
routes_dir = './simulation/leaderboard/data/training_routes/splitted_routes'
...
scen_params = 'leaderboard/scenarios/scenario_parameter_datagen.yaml'
    ...
    for town in town_list:
        ...
        for i, port_len in enumerate(town_dict[town]['port_split']):
            ...
            for _ in range(port_len):
                ...
                fw.write("bash ./simulation/data_collection/scripts/weather-%d/%s.sh \n" % (weather, route))
                route_order += 1

            fw_all.write("bash ./simulation/data_collection/batch_run/weather-%d/town_%s_%s.sh & \n" % (weather,town,i))
