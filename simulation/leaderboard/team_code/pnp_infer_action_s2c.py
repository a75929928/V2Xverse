...
plt.ioff() # disable the interface
...
class PnP_infer():
	def __init__(self, config=None, ego_vehicles_num=1, perception_model=None, planning_model=None, perception_dataloader=None, device=None) -> None:
		...
		# NOTE ARGS REVISED to fit with s2c
		voxel_args = {
			'args': {
				'voxel_size': [0.4, 0.4, 36], 
				'max_points_per_voxel': 32,
				'max_voxel_train': 70000,
				'max_voxel_test': 40000
			},
			'cav_lidar_range': [-115.2, -38.4, -22, 115.2, 38.4, 14]   # x_min, y_min, z_min, x_max, y_max, z_max
		}
	...
	def get_action_from_list_inter(self, car_data_raw, rsu_data_raw, step, timestamp, skipped=False):
		...
		## Revise predefined latency to real latency with its time_delay
		# NOTE only found time_delay(not comm_latency)
		if True:
			car_data_out = [] # directly organize raw data with past ones cause repeat data
			rsu_data_out = []
		# if 'comm_latency' in self.config['simulation']:
			...
			latency_step = 4 # reserve data from 4 frames past (4*50ms = 200ms)
			# latency_step = self.config['simulation']['comm_latency']

			print('step:', step)
			# print('len_latency_bank:', len(self.pre_raw_data_bank.keys()))
			...
			if step > latency_step:
				...
				for sorted_key in sorted_keys: # timestamp list
					if sorted_key not in self.pre_raw_data_bank: 
					# if sorted_key not in self.pre_raw_data_bank or sorted_key == step: 
						continue
					# self.pre_raw_data_bank[sorted_key].update({'time_delay': step-sorted_key})
					# remember car_data is stored as list
					for cav_id, car_data in enumerate(self.pre_raw_data_bank[sorted_key]['car_data']): 
						car_data.update({'time_delay': step-sorted_key})
						car_data_out.append(car_data)
					for rsu_id, rsu_data in enumerate(self.pre_raw_data_bank[sorted_key]['rsu_data']):
						rsu_data.update({'time_delay': step-sorted_key})
						rsu_data_out.append(rsu_data)

				# car_0 should always be the first element
				car_data_raw = car_data_out
				rsu_data_raw = rsu_data_out

				car_data_raw.sort(key=lambda x: x['time_delay'])
				rsu_data_raw.sort(key=lambda x: x['time_delay'])
