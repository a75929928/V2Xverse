...
from team_code.pnp_infer_action_s2c import PnP_infer # NOTE revised voxel settings
...
class PnP_Agent(autonomous_agent.AutonomousAgent):
    ...
    def spawn_sensors(self):
        """
        spawn sensor at background vehicles and delete old ones
        """
        if self.step % self.config['RSU']['change_rsu_frame'] == 0:
            if self.first_generate_rsu:
                self.first_generate_rsu = False
    ...
    def run_step(self, input_data: dict, timestamp):
        ...
        # spawn the rsu.
        self.spawn_rsu()
        # NOTE also add obs for background vehicles
        self.spawn_sensors()
        rsu_data = []
